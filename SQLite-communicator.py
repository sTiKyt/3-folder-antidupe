...
def load_db_to_list(db_name): #TODO make
    db_list = [[],[],[]]
    conn = sqlite3.connect(db_name)
    c = conn.cursor()
    c.execute('')
...
def merge_sha256_with_db(total):
    progress = 0
    conn = sqlite3.connect(db_name)
    c = conn.cursor()
...
if not first_launch:
    db_list = load_db_to_list(db_name)#Then database lists get loaded in
...
merge_sha256_with_db(total)#Then the cleaned sha256 list gets merged into the database
